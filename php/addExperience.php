...
<html lang="en">
    ...
    $companyNameUpdate=$_POST['univName'];  
    $joiningUpdate=$_POST['startYear'];
    $endUpdate=$_POST['endYear'];  
    $roleUpdate=$_POST['degree'];
    $descriptionUpdate=$_POST['major']; 
    
    $sql="INSERT INTO workexp (companyName, joiningDate, endDate,role,description,num)
VALUES ('".$companyNameUpdate."', '".$joiningUpdate."', '".$endUpdate."','".$roleUpdate."','".$descriptionUpdate."',1)"; 
  
    if ($conn->query($sql) === TRUE) { 
    echo '<script type="text/javascript">'; 
  echo 'alert("Inserted Experience successfully");'; 
  ...
  echo '</script>';   
    ...
    $err="All fields are required!";  
        ...
        <form action="" method='POST'>
          <label for="univName">Company Name</label>
          <input type="text" id="univName" name="univName" class='signText' required>
          <label for="startYear">Joining Date</label>
          ...
          <input type="text" id="degree" name="degree" class='signText' required>
            <label for="major">Responsibilities</label>
